...
def main():
    ...
    bak=a.infile+".bak"; shutil.copy2(a.infile,bak)
    print(f"[{now()}] Backup created: {bak}")
    print(f"[{now()}] Checking {len(rows)} targets with {a.workers} workers...\n")

    alive_rows=[]; dead=0; total=len(rows)
    with ThreadPoolExecutor(max_workers=a.workers) as ex:
        futs={ex.submit(alive,r,a.timeout):r for r in rows}
        for i,fut in enumerate(as_completed(futs),1):
            r=futs[fut]; t=target(r); ok=fut.result()
            if ok: alive_rows.append(r)
            else: dead+=1
            print(f"[{i:>4}/{total}] {t:<30} {'ALIVE' if ok else 'DEAD'}")

    write_csv(a.infile,alive_rows,headers)
    print(f"\n[{now()}] Finished. Alive: {len(alive_rows)} | Dead: {dead} | Total: {total}")
